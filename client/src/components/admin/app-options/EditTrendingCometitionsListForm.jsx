...
import { findTrendingCompetitions } from "../../../app/helpers/competitions-algorithms";
import { Link } from "react-router-dom";
import ErrorMessage from "../../shared/ErrorMessage";
import SuccessMessage from "../../shared/SuccessMessage";

export default function EditTrendingCompetitionsListForm() {
  ...
  useEffect(() => {
    if (filteredCompetitions) {
      const arr = findTrendingCompetitions(competitions);
  ...
  const removeFeaturedCompetition = async (comp) => {
    ...
    setSuccess("");
    setWait(true);
    const r = await dispatch(updateCompetition(comp?.ID, { trending: false }));
  ...
  return (
    <>
      {wait ? (
        ...
        <>
          <h4>Edit Trending Competitions List</h4>
